...
function headerServices() {
	...
	var linksNodes = menu.querySelectorAll('.services-menu__link');
	var links = Array.prototype.slice.call(linksNodes);
	var linkActiveClass = 'services-menu__link_active';

	var sticky = new Sticky('#services-menu', {
		marginTop: 116,
		stickyContainer: '#services'
	});

	function linksActiveControl() {
		var screenH = window.innerHeight;
		var st = window.pageYOffset;
		var activeLinkIndex = links.length - 1;
		for (var i = 0; i < links.length; i++) {
			var nodeId = links[i].getAttribute("href").replace('#', '');
			var node = document.getElementById(nodeId);
			if (node) {
				if (node.offsetTop > st + screenH*0.3333) {
					activeLinkIndex = Math.max(0, i - 1);
					break;
				}
			}
		}
		...
		for (var i = 0; i < links.length; i++) {
			links[i].classList.remove(linkActiveClass);
		}
		links[activeLinkIndex].classList.add(linkActiveClass);
	}

	window.addEventListener('scroll', linksActiveControl);
}

document.addEventListener("DOMContentLoaded", headerServices);

"use strict";

function smoothScroll() {
	var selector = 'a.js-smooth-scroll';
	var linksNodes = document.querySelectorAll(selector);
	var links = Array.prototype.slice.call(linksNodes);
	...
	for (var i = 0; i < links.length; i++) {
		links[i].addEventListener('click', function(e) {
			...
			var offset = parseInt()
			if (node) {
				var top = node.offsetTop - headerHeight - 15;
				window.scrollTo({
					top: top,
					behavior: "smooth"
				});
